...
// Phil Ero 15JUL08
    ...
    public class BumperCollision
   {
       ...
       private static void findImpactPoint(Bumper bumper, Ball ball)
      {
         ...
         for (int x = (int) bumper.getX(); x <= bumper.getX() + bumper.getXWidth(); x++)  // top & bottom edges
         {
            updateIfCloser(x, (int)bumper.getY(), ball);
            updateIfCloser(x, (int)bumper.getY() + (int)bumper.getYWidth(), ball);
         }
         for (int y = (int) bumper.getY(); y <= bumper.getY() + bumper.getYWidth(); y++)  // right & left edges
         {
            updateIfCloser((int)bumper.getX(), y, ball);
            updateIfCloser((int)bumper.getX() + (int)bumper.getXWidth(), y, ball);
         }
      }
